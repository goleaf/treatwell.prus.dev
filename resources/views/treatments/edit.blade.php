    ...
    <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Edit Treatment</h1>
                    <p class="mt-2 text-gray-600 dark:text-gray-400">Update treatment information</p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
            <form method="POST" action="{{ route('web.treatments.update', $treatment) }}" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="venue_id" 
                        ...
                        @foreach($venues as $venue)
                            <option value="{{ $venue->id }}" {{ old('venue_id', $treatment->venue_id) == $venue->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <textarea name="description" 
                              id="description"
                              rows="4"
                              class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('description') border-red-500 @enderror">{{ old('description', $treatment->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" 
                           name="category_name" 
                           id="category_name"
                           value="{{ old('category_name', $treatment->category_name) }}"
                           list="categories"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" 
                                   name="min_price" 
                                   id="min_price"
                                   value="{{ old('min_price', $treatment->min_price) }}"
                                   step="0.01"
                                   min="0"
                        ...
                        <div>
                            ...
                            <input type="number" 
                                   name="max_price" 
                                   id="max_price"
                                   value="{{ old('max_price', $treatment->max_price) }}"
                                   step="0.01"
                                   min="0"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" 
                                   name="min_duration" 
                                   id="min_duration"
                                   value="{{ old('min_duration', $treatment->min_duration) }}"
                                   min="1"
                        ...
                        <div>
                            ...
                            <input type="number" 
                                   name="max_duration" 
                                   id="max_duration"
                                   value="{{ old('max_duration', $treatment->max_duration) }}"
                                   min="1"
                ...
                <div class="flex items-center justify-between pt-6 border-t border-gray-200 dark:border-gray-700">
                    <div>
                        <button type="button" 
                                onclick="if(confirm('Are you sure you want to delete this treatment?')) { document.getElementById('delete-form').submit(); }"
            ...
            <!-- Hidden Delete Form -->
            <form id="delete-form" method="POST" action="{{ route('web.treatments.destroy', $treatment) }}" class="hidden">
                @csrf
                @method('DELETE')
            </form>
        </div>
